...
questionData= function(objJSON,callback){
 ...
 naturalLanguage = function(question,array){
	let originalQuestion = question.toString().trim().toLowerCase();
	let findInput = 0;
	let findIndex = 0;

	let documents = this.caputuraDeDados.getDocuments(originalQuestion);
	...
	if(documents){
		...
		for(let i=0; i<array.length; i++){
			question = question.toString().trim().toLowerCase();
			let input = array[i].input.toString().trim().toLowerCase();
	
			if(input.length<=0){
				input = array[i].output.toString().trim().toLowerCase();
			}
